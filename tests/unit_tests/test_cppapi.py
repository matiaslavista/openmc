...
@pytest.fixture(scope='module')
def core_init(pincell_model):
    openmc.core.init()
    yield
    openmc.core.finalize()


@pytest.fixture(scope='module')
def core_simulation_init(core_init):
    openmc.core.simulation_init()
    yield


@pytest.fixture(scope='module')
def core_run(core_simulation_init):
    openmc.core.run()
'''
...
def test_source_bank(core_run):
    ...
    #assert np.all(source_arr['E'] > 0.0)
    #assert np.all(source_arr['wgt'] == 1.0)
    #assert np.allclose(np.linalg.norm(source_arr['u'], axis=1), 1.0)
'''
